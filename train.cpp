...
int main()
{
	// Getting camera access, checking if image capture works
	...
	while(1)
	{
		...
		string prefix;
		cin >> prefix;
		printf("Enter folder name to save as (same as image prefix): \n");
		while(1)
		{
			...
			switch(c)
			{
				...
				if(faces.size())
				{
					...
					Mat myMat(temp);
					char buffer[100];
					sprintf(buffer, "./apoorv/sad/apoorv_sad_%d.png", imageSaveCounter++);
